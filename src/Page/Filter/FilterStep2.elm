...
update msg model =
    case msg of 
        ...
        GoRegist ->
            ...
            (model, registVideo model item model.session)
        UpdateTitle title ->
            ({model | title = title}, Cmd.none)
        ReceiveDataFromStorage data ->
            let _ = Debug.log "data" data
        ...
        Expand (idx, set)->
            if model.style == String.fromInt(idx) then
                ({model | style = "", setData = set}, Cmd.none)
            else
                ({model | style = String.fromInt(idx), setData = set}, Cmd.none)
...
app model =
    div [] [
        appHeaderBackComplete "운동추가" "makeExerHeader" BackBtn GoRegist
...
appselectedItem idx item model=
    div [ class "m_filterstep2_videolistbox" ]
        [ div [ class "m_yf_switch" ]
            [ img [ src "/image/m_down.png", class "m_downIcon" ]
                []
            , img [ src "/image/m_up.png", class "m_upIcon"]
                []
            ]
    ...
    div [ classList [
        ...
        , div [ class "filterstep2_yf_setting2" ]
            [ div [ class "button filterstep2_mbtn", onClick (Minus idx) ]
                [ text "-" ]
            , input [ class "input filterstep2_yf_num", type_ "text", value (String.fromInt(model.setData)), onInput UpdateSet]
    ...
    div [ classList [
        ...
        , div [ class "m_filterstep2_yf_setting2" ]
            [ div [ class "button m_filterstep2_mbtn", onClick (Minus idx) ]
                [ text "-" ]
            , input [ class "input m_filterstep2_yf_num", type_ "text", value (String.fromInt(model.setData)),
             onInput UpdateSet]
                [text (String.fromInt(model.setData))], 
                 if justokData rest == "" then text "분"  else text "회"
